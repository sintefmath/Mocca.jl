...
# function plot_cell(model,states,timesteps,cell,pvar)
#     key_to_label = Dict(
#         :y => "y",
#         :Pressure => "p",
#         :AdsorbedConcentration => "q",
...
#     for (nsymb, symbol) in enumerate(pvars)
#         @show symbol
#         f[end] = CairoMakie.Figure()
#         if size(states[cell][symbol], 2) == 1
#             ax = CairoMakie.Axis(f[nsymb, 1], title=String(symbol), xlabel=CairoMakie.L"t", ylabel=CairoMakie.L"%$(key_to_label[symbol])")
#             CairoMakie.lines!(ax, t, Float64.([result[symbol][cell] for result in states]), color=:darkgray)
#         else
#             ax = CairoMakie.Axis(f[nsymb, i], title=String(symbol), xlabel=CairoMakie.L"t", ylabel=CairoMakie.L"%$(key_to_label[symbol])")
#             for i in 1:size(states[cell][symbol], 1)
#                 CairoMakie.lines!(ax, t, Float64.([result[symbol][i, cell] for result in states]), color=:darkgray)
#             end
#         end
#         f
#     end
